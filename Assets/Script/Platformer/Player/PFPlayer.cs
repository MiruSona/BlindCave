...
public class PFPlayer : MonoBehaviour {
    ...
    //빛
    private float light_max = 0.2f;
    private float light_delta = 0.002f;
    ...
    {
        //플레이 상태 아니면 중지
        if (game_manager.state == GameManager.State.Goal)
        {
    ...
    private void OnCollisionEnter2D(Collision2D _col)
    {
        ...
        {
            if(-0.1f < rb2d.velocity.y && rb2d.velocity.y < 0.1f)
            {
                //빛나게
    ...
    private void MoveStop()
    {
        ...
        if (player_data.state == PlayerData.State.Attacked && !player_data.ground)
            return;
    ...
    private void Attacked()
    {
        ...
        if (attacked_timer.time == 0)
        {
            ...
            player_data.ground = false;
            Vector2 force = new Vector2(-player_data.facing, 1f) * push_back;
            rb2d.velocity = force;
        }
